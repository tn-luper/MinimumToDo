            ...
            @foreach ($pastcounts as $pastkey => $pastvalue)
                ...
                @foreach ($status1counts as $status1key => $status1value)
                    @if($status1key === $pastkey)
                        <div class="progress-bar bg-success" role="progressbar" style="width: {{$status1value / $pastvalue * 100}}%"><font color="black" font size="3">{{$status1value}}</font></div>
                    @endif
                @endforeach
                @foreach ($status2counts as $status2key => $status2value)
                    @if($status2key === $pastkey)
                        <div class="progress-bar bg-warning" role="progressbar" style="width: {{$status2value / $pastvalue * 100}}%"><font color="black" font size="3">{{$status2value}}</font></div>
                    @endif
                @endforeach
                @foreach ($status3counts as $status3key => $status3value)
                    @if($status3key === $pastkey)
                        <div class="progress-bar bg-danger" role="progressbar" style="width: {{$status3value / $pastvalue * 100}}%"><font color="black" font size="3">{{$status3value}}</font></div>
                    @endif
                @endforeach
                </div>
            @endforeach
