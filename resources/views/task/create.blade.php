    ...
    <div>
        ...
        <form action="/active" method="POST">
            @csrf
            <p>
            実施日：<input type="date" name="task[date]" value="{{old('task.date')}}">
            </p>
            <p class="date__error" style="color:red">{{ $errors->first('task.date') }}</p>
            <p>
            タスクの内容：<input type="text" name="task[task_name]" value="{{old('task.task_name')}}">
            </p>
            <p class="task_name__error" style="color:red">{{ $errors->first('task.task_name') }}</p>
            <p>
            タスクを行うタイミング：<input type="text" name="task[start_time]" value="{{old('task.start_time')}}">
            </p>
            <p class="start_time__error" style="color:red">{{ $errors->first('task.start_time') }}</p>
            <p>
            最低限やること(must)：<input type="text" name="task[bare_minimum]" value="{{old('task.bare_minimum')}}">
            </p>
            <p class="bare_minimum__error" style="color:red">{{ $errors->first('task.bare_minimum') }}</p>
            <p>
            できたらやること(want)：<input type="text" name="task[target_amount]" value="{{old('task.target_amount')}}">
            </p>
            <p class="target_amount__error" style="color:red">{{ $errors->first('task.target_amount') }}</p>
            <p>
            工夫：<input type="text" name="task[plan]" value="{{old('task.plan')}}">
            </p>
